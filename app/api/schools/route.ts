...
const getSchoolMetadata = unstable_cache(
  async () => {
    ...
    return { academic_groups: academic_groups.sort(), colleges: colleges.sort(), regions, cities: cities.sort() };
  },
  ['school-metadata'], 
...
export async function GET(request: Request) {
  try {
    ...
    const projection = {
      school_id: 1, school_name: 1, school_type: 1, school_url: 1, school_images: 1, 
      campuses: 1,
      // 僅回傳部門摘要
      "departments.department_id": 1, "departments.department_name": 1,
      "departments.college": 1, "departments.academic_group": 1,
      "departments.campus_ids": 1, "departments._id": 1,
    ...
    if (!hasAdvancedFilters) {
      ...
      if (region) {
        ...
        query.campuses = { $elemMatch: { is_main: true, 'location.city': region } };
      }
      total = await School.countDocuments(query);
      schools = await School.find(query).select(projection).skip(skip).limit(limit);
      ...
      if (region) {
        matchStage.campuses = { $elemMatch: { is_main: true, 'location.city': region } };
      }
        ...
        $addFields: {
          __year_entry: {
            $arrayElemAt: [
              { 
                $filter: { 
                  input: { $objectToArray: { $ifNull: ["$departments.admission_data", {}] } }, 
                  as: "item", 
                  cond: { $eq: ["$$item.k", year] } 
                } 
              }, 
      ...
      pipeline.push({
        $addFields: {
          __target_plan: {
            $switch: {
              branches: [
                { case: { $eq: [planKey, 'star_plan'] }, then: "$__year_entry.v.plans.star_plan" },
                { case: { $eq: [planKey, 'personal_application'] }, then: "$__year_entry.v.plans.personal_application" },
                { case: { $eq: [planKey, 'distribution_admission'] }, then: "$__year_entry.v.plans.distribution_admission" }
              ],
              default: null
            }
          }
           ...
           $lte: [
             {
               $switch: {
                 branches: [
                   { case: { $eq: [{ $ifNull: ["$__target_plan.english_listening_threshold", "F"] }, 'A'] }, then: 4 },
                   { case: { $eq: [{ $ifNull: ["$__target_plan.english_listening_threshold", "F"] }, 'B'] }, then: 3 },
                   { case: { $eq: [{ $ifNull: ["$__target_plan.english_listening_threshold", "F"] }, 'C'] }, then: 2 },
                   { case: { $eq: [{ $ifNull: ["$__target_plan.english_listening_threshold", "F"] }, 'F'] }, then: 1 },
                   { case: { $eq: [{ $ifNull: ["$__target_plan.english_listening_threshold", "F"] }, '--'] }, then: 0 },
                   { case: { $eq: [{ $ifNull: ["$__target_plan.english_listening_threshold", "F"] }, '無'] }, then: 0 },
                 ],
                 default: 0 
               }
             },
      ...
      if (hasScores) {
                            ...
                            $lt: [
                              {
                                ...
                                $switch: {
                                  branches: [
                                    { case: { $eq: ["$$subjName", "國文"] }, then: userLevels["國文"] ?? 0 },
                                    { case: { $eq: ["$$subjName", "英文"] }, then: userLevels["英文"] ?? 0 },
                                    { case: { $eq: ["$$subjName", "數學A"] }, then: userLevels["數學A"] ?? 0 },
                                    { case: { $eq: ["$$subjName", "數學B"] }, then: userLevels["數學B"] ?? 0 },
                                    { case: { $eq: ["$$subjName", "自然"] }, then: userLevels["自然"] ?? 0 },
                                    { case: { $eq: ["$$subjName", "社會"] }, then: userLevels["社會"] ?? 0 },
                                  ],
                              ...
                              },
                              {
                                ...
                                $switch: {
                                  branches: [
                                    { case: { $eq: ["$$reqLevelStr", "頂標"] }, then: 5 },
                                    { case: { $eq: ["$$reqLevelStr", "前標"] }, then: 4 },
                                    { case: { $eq: ["$$reqLevelStr", "均標"] }, then: 3 },
                                    { case: { $eq: ["$$reqLevelStr", "後標"] }, then: 2 },
                                    { case: { $eq: ["$$reqLevelStr", "底標"] }, then: 1 }
                                  ],
                                  default: 0 
                                }
                              }
        ...
        if (advancedMatch["$expr"]) {
          advancedMatch["$expr"] = { $and: [advancedMatch["$expr"], thresholdCheckExpr] };
        } else {
          advancedMatch["$expr"] = thresholdCheckExpr;
        }

        advancedMatch["__target_plan.exam_thresholds"] = { $exists: true, $ne: [] };
      }
      ...
      const facetStage = {
        $facet: {
          ...
          data: [{ $sort: { school_id: 1 } }, { $skip: skip }, { $limit: limit }]
        }
      };
